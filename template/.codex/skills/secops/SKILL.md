...
### Codex Permissions Note

Codex does not use `.claude/settings.json`, so there is no Codex-specific permission wiring to mirror here.
If `osv-scanner` requires approval in the current environment, request it before proceeding.
