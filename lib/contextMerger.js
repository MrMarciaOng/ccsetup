...
class ContextMerger {
  ...
  mergeSection(sectionName, existingSection, newSection, strategy = 'smart') {
    ...
    switch (strategy) {
      case 'union':
        return this.mergeWithUnion(existingData, newData, sectionName);
      case 'smart':
      ...
      case 'update-scan':
        ...
        return existingData;
      case 'interactive':
        // Interactive mode will be handled at a higher level
        return { existingData, newData, needsResolution: true };
      default:
        return newData;
    }
  }

  mergeWithUnion(existing, newData, sectionName) {
  ...
  async merge(strategy = 'smart') {
    ...
    return this.smartMerge(strategy);
  }
  ...
  async reconstructMarkdown(sections) {
    ...
    // Try to get AI-suggested section order
    const aiOrder = await this.aiHelper.suggestSectionOrder(sections);
    
    const sectionOrder = aiOrder || [
      'Project Overview',
      'Tech Stack',
      'Key Commands',
      'Project Structure',
    ...
    if (aiOrder) {
      console.log('   🤖 Using AI-optimized section order');
    }
